...
#include "TraversalRootMotionModifier_PrecomputedWarp.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(TraversalRootMotionModifier_PrecomputedWarp)

UTraversalRootMotionModifier_PrecomputedWarp::UTraversalRootMotionModifier_PrecomputedWarp(const FObjectInitializer& ObjectInitializer)
...
namespace
{
	// Cubic easing (same family as legacy AHEasing / warrenm/AHEasing); avoids Engine ThirdParty path drift across UE versions.
...
float UTraversalRootMotionModifier_PrecomputedWarp::GetWeight(float CurrentTime, const FTraversalPrecomputedWarpCurve& WarpCurve) const
{
	float Result;

	if (WarpCurve.CurveType == ETraversalPrecomputedWarpWeightCurveType::FromRootMotionTranslation && !AnimTrajectoryData.TranslationCurve.IsEmpty())
	{
	...
	else if (WarpCurve.CurveType == ETraversalPrecomputedWarpWeightCurveType::FromRootMotionRotation && !AnimTrajectoryData.RotationCurve.IsEmpty())
	{
		const float Duration = (EndTime - StartTime);
		const float StartCurveSampleTime = FMath::Max(WarpCurve.StartRatio * Duration, ActualStartTime) - ActualStartTime;
	...
	else
	{
		...
		{
			case ETraversalPrecomputedWarpWeightCurveType::EaseIn:
				Result = CubicEaseIn(CurrentRelativeTime);
				break;
			case ETraversalPrecomputedWarpWeightCurveType::EaseOut:
				Result = CubicEaseOut(CurrentRelativeTime);
				break;
			case ETraversalPrecomputedWarpWeightCurveType::EaseInOut:
				Result = CubicEaseInOut(CurrentRelativeTime);
				break;
			case ETraversalPrecomputedWarpWeightCurveType::Instant:
				Result = 1.f;
				break;
			case ETraversalPrecomputedWarpWeightCurveType::DoNotWarp:
				Result = 0.f;
				break;
			default:
...
FTransform UTraversalRootMotionModifier_PrecomputedWarp::ProcessRootMotion(const FTransform& InRootMotion, float DeltaSeconds)
{
	float CurrentTime = CurrentPosition;
	UTraversalRootMotionModifier_PrecomputedWarp* AlignmentNotify = this;
	...
	if (!WarpedTrajectory.IsEmpty())
	{
		...
		if (PreviousFrame < WarpedTrajectory.Num() - 1)
		{
			...
			if(AlignmentNotify->UpdateMode == ETraversalPrecomputedWarpUpdateMode::World)
			{
			...
			{
				OutRootMotion.SetRotation(FQuat::Identity);
			}
