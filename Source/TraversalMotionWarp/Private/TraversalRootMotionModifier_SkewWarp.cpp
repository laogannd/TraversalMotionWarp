...
#include UE_INLINE_GENERATED_CPP_BY_NAME(TraversalRootMotionModifier_SkewWarp)

UTraversalRootMotionModifier_SkewWarp::UTraversalRootMotionModifier_SkewWarp(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

FVector UTraversalRootMotionModifier_SkewWarp::WarpTranslation(const FTransform& CurrentTransform, const FVector& DeltaTranslation, const FVector& TotalTranslation, const FVector& TargetLocation)
{
...
FTransform UTraversalRootMotionModifier_SkewWarp::ProcessRootMotion(const FTransform& InRootMotion, float DeltaSeconds)
{
	...
	{
		ExtraRootMotion = UTraversalMotionWarpUtilities::ExtractRootMotionFromAnimation(Animation.Get(), EndTime, CurrentPosition);
	}
...
UTraversalRootMotionModifier_SkewWarp* UTraversalRootMotionModifier_SkewWarp::AddRootMotionModifierSkewWarp(UTraversalMotionWarpComponent* InMotionWarpingComp, const UAnimSequenceBase* InAnimation, float InStartTime, float InEndTime,
	...
	bool bInWarpTranslation, bool bInIgnoreZAxis, bool bInWarpRotation, ETraversalMotionWarpRotationType InRotationType, ETraversalMotionWarpRotationMethod InRotationMethod, float InWarpRotationTimeMultiplier, float InWarpMaxRotationRate, bool bInIgnoreRootMotionRotation)
{
	if (ensureAlways(InMotionWarpingComp))
	{
		UTraversalRootMotionModifier_SkewWarp* NewModifier = NewObject<UTraversalRootMotionModifier_SkewWarp>(InMotionWarpingComp);
...
FTransform UTraversalRootMotionModifier_SkewWarp::GetDebugWarpPointTransform(USkeletalMeshComponent* MeshComp, const UAnimSequenceBase* InAnimation, const UMirrorDataTable* MirrorTable, const float NotifyEndTime) const
{
	...
	else if (WarpPointAnimProvider == ETraversalWarpPointAnimProvider::Static)
	{
	...
	else if (WarpPointAnimProvider == ETraversalWarpPointAnimProvider::Bone)
	{
		if (const UAnimInstance* AnimInstance = MeshComp->GetAnimInstance())
		{
			...
			if (BoneIndex != INDEX_NONE)
			{
				...
				UTraversalMotionWarpUtilities::ExtractComponentSpacePose(InAnimation, LimitedBoneContainer, NotifyEndTime, false, Pose);
...
void UTraversalRootMotionModifier_SkewWarp::DrawInEditor(FPrimitiveDrawInterface* PDI, USkeletalMeshComponent* MeshComp, const UAnimSequenceBase* InAnimation, const FAnimNotifyEvent& NotifyEvent) const
{
...
void UTraversalRootMotionModifier_SkewWarp::DrawCanvasInEditor(FCanvas& Canvas, FSceneView& View, USkeletalMeshComponent* MeshComp, const UAnimSequenceBase* InAnimation, const FAnimNotifyEvent& NotifyEvent) const
{
	// Early out if the animation does not have root motion.
	if (!InAnimation || !InAnimation->HasRootMotion())
	{
