...
#include "TraversalRootMotionModifier.generated.h"
 ...
 */
USTRUCT()
struct FTraversalMotionWarpUpdateContext
{
	GENERATED_BODY()
...
UENUM(BlueprintType)
enum class ETraversalRootMotionModifierState : uint8
{
...
class UTraversalRootMotionModifier : public UObject
{
	GENERATED_BODY()
...
public:
	...
	UPROPERTY()
	FOnTraversalRootMotionModifierDelegate OnActivateDelegate;

	/** Delegate called when this modifier updates while active (affecting the root motion) */
	UPROPERTY()
	FOnTraversalRootMotionModifierDelegate OnUpdateDelegate;

	/** Delegate called when this modifier is deactivated (stops affecting the root motion) */
	UPROPERTY()
	FOnTraversalRootMotionModifierDelegate OnDeactivateDelegate;
	...
	UE_API void SetState(ETraversalRootMotionModifierState NewState);

	/** Returns the state of the modifier */
	inline ETraversalRootMotionModifierState GetState() const { return State; }

	/** Returns a pointer to the component that owns this modifier */
	UE_API UTraversalMotionWarpComponent* GetOwnerComponent() const;

	/** Returns a pointer to the adapter to the object being warped */
	UE_API UTraversalMotionWarpBaseAdapter* GetOwnerAdapter() const;
	...
	UE_API virtual void Update(const FTraversalMotionWarpUpdateContext& Context);
...
private:
	...
	friend UTraversalMotionWarpComponent;

	/** Current state */
	UPROPERTY()
	ETraversalRootMotionModifierState State = ETraversalRootMotionModifierState::Waiting;
};

UENUM(BlueprintType, meta=(Experimental))
enum class ETraversalWarpTargetLocationOffsetDirection : uint8
{
...
// UTraversalRootMotionModifier_Warp
///////////////////////////////////////////////////////////////

/** Represents a point of alignment in the world */
USTRUCT(BlueprintType, meta = (HasNativeMake = "/Script/TraversalMotionWarp.TraversalMotionWarpFunctionLibrary.MakeTraversalMotionWarpTarget"))
struct FTraversalMotionWarpTarget
{
	...
	/** Determines how LocationOffset is applied by FTraversalMotionWarpTarget when calculating a target transform
	Only relevant when the warp target is created from a component */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Experimental)
	ETraversalWarpTargetLocationOffsetDirection LocationOffsetDirection = ETraversalWarpTargetLocationOffsetDirection::TargetsForwardVector;
	...
	FTraversalMotionWarpTarget(const FName& InName, const FTransform& InTransform)
	...
	UE_API FTraversalMotionWarpTarget(const FName& InName, const USceneComponent* InComp, FName InBoneName, bool bInbFollowComponent, ETraversalWarpTargetLocationOffsetDirection InLocationOffsetDirection = ETraversalWarpTargetLocationOffsetDirection::TargetsForwardVector, const AActor* InAvatarActor = nullptr, const FVector& InLocOffset = FVector::ZeroVector, const FRotator& InRotOffset = FRotator::ZeroRotator);
	
	// @TODO: Deprecate this.
	UE_API FTraversalMotionWarpTarget(const FName& InName, const USceneComponent* InComp, FName InBoneName, bool bInbFollowComponent, const FVector& InLocOffset = FVector::ZeroVector, const FRotator& InRotOffset = FRotator::ZeroRotator);
	...
	inline bool operator==(const FTraversalMotionWarpTarget& Other) const
	{
...
UENUM(BlueprintType)
enum class ETraversalMotionWarpRotationType : uint8
{
...
UENUM(BlueprintType)
enum class ETraversalMotionWarpRotationMethod : uint8
{
...
UENUM(BlueprintType)
enum class ETraversalWarpPointAnimProvider : uint8
{
...
class UTraversalRootMotionModifier_Warp : public UTraversalRootMotionModifier
{
	GENERATED_BODY()
...
public:
	...
	ETraversalWarpPointAnimProvider WarpPointAnimProvider = ETraversalWarpPointAnimProvider::None;

	//@TODO: Hide from the UI when Target != Static
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Config", meta = (EditCondition = "WarpPointAnimProvider == ETraversalWarpPointAnimProvider::Static"))
	FTransform WarpPointAnimTransform = FTransform::Identity;

	//@TODO: Hide from the UI when Target != Bone
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Config", meta = (EditCondition = "WarpPointAnimProvider == ETraversalWarpPointAnimProvider::Bone"))
	...
	/** Whether to completely ignore the rotation component of root motion (both warped and original animation rotation).
	...
	ETraversalMotionWarpRotationType RotationType = ETraversalMotionWarpRotationType::Default;
	
	/** The method of rotation to use */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Config", meta = (EditCondition = "bWarpRotation"))
	ETraversalMotionWarpRotationMethod RotationMethod = ETraversalMotionWarpRotationMethod::Slerp;
	 ...
	 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Config", meta = (EditCondition = "RotationMethod!=ETraversalMotionWarpRotationMethod::ConstantRate && RotationMethod!=ETraversalMotionWarpRotationMethod::Scale && bWarpRotation"))
	...
	UE_API virtual void Update(const FTraversalMotionWarpUpdateContext& Context) override;
	...
	UE_API virtual void OnStateChanged(ETraversalRootMotionModifierState LastState) override;
...
// DEPRECATED in favor of UTraversalRootMotionModifier_SkewWarp (kept for reference)
///////////////////////////////////////////////////////////////

UCLASS(MinimalAPI, Deprecated, meta = (DisplayName = "Simple Warp"))
class UDEPRECATED_TraversalRootMotionModifier_SimpleWarp : public UTraversalRootMotionModifier_Warp
{
	GENERATED_BODY()

public:

	UE_API UDEPRECATED_TraversalRootMotionModifier_SimpleWarp(const FObjectInitializer& ObjectInitializer);
...
class UTraversalRootMotionModifier_Scale : public UTraversalRootMotionModifier
{
	GENERATED_BODY()
...
public:
	...
	UTraversalRootMotionModifier_Scale(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer) {}
	...
	static UE_API UTraversalRootMotionModifier_Scale* AddRootMotionModifierScale(
